...
class PopoverKeyPad extends Component {
  ...
  control(isIncrement) {
    const { decimalSeparator, max, min } = this.props;
    this.setState(prevState => {
      const prevVal = prevState.input.replace(decimalSeparator, '.') * 1;
      const temp = prevVal + (isIncrement ? this.step : -this.step);
      ...
      if (this.step !== 1 && newVal % this.step !== 0) {
        newVal = newVal > prevVal ? prevVal + this.step : prevVal - this.step;
      }
